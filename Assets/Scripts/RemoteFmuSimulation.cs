...
///
/// Input writes are buffered and KEPT across steps so the last known
/// value of every variable is always sent. Call SetBoolean/SetReal
...
public class RemoteFmuSimulation : MonoBehaviour, ISimulationBackend
{
    ...
    public void SetBoolean(string name, bool value)  => _inputs[name] = value;
    public void SetReal   (string name, double value) => _inputs[name] = value;
    public void SetInteger(string name, int value)    => _inputs[name] = value;

    public float GetReal(string name) =>
        _outputs.TryGetValue(name, out var v) ? Convert.ToSingle(v) : 0f;
    public bool GetBoolean(string name) =>
        _outputs.TryGetValue(name, out var v) && Convert.ToBoolean(v);
    public int GetInteger(string name) =>
        _outputs.TryGetValue(name, out var v) ? Convert.ToInt32(v) : 0;
    ...
    // _inputs is PERSISTENT — never cleared, only overwritten.
    // This ensures every step sends the current state of all variables.
    readonly Dictionary<string, object> _inputs  = new();
    readonly Dictionary<string, object> _outputs = new();
    ...
    IEnumerator DoStep(float dt)
    {
        ...
        // Send the full current state of all inputs (no snapshot/clear).
        var inputsJson = BuildInputsJson(_inputs);
    ...
    IEnumerator DoReset()
    {
        ...
        if (req.result == UnityWebRequest.Result.Success)
        {
            _inputs.Clear(); // reset inputs to default on sim reset

...
public class RemoteFmuSimulation : MonoBehaviour, ISimulationBackend
{
    ...
    IEnumerator DoStep(float dt)
    {
        ...
        var body =
            $"{{\"session_id\":\"{_sessionId}\",\"dt\":{dt.ToString("F4", System.Globalization.CultureInfo.InvariantCulture)},\"inputs\":{inputsJson}}}";
        var bytes = Encoding.UTF8.GetBytes(body);
        ...
        foreach (var kv in dict)
        {
            ...
            switch (kv.Value)
            {
                ...
                case int i:
                    ...
                    break;
                case float f:
                    sb.Append(f.ToString("F6", System.Globalization.CultureInfo.InvariantCulture));
                    break;
                case double d:
                    sb.Append(d.ToString("F6", System.Globalization.CultureInfo.InvariantCulture));
                    break;
                default:
                    sb.Append($"\"{kv.Value}\"");
                    break;

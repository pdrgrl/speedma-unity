...
namespace Speedma.Editor
{
    public class FmuTesterWindow : EditorWindow
    {
        ...
        private void Tick()
        {
            ...
            {
                if (!s.AutoStep || s.Requesting || string.IsNullOrEmpty(s.SessionId))
                    continue;
        ...
        private void EnsureStyles()
        {
            ...
            {
                normal = { textColor = new Color(0.2f, 0.85f, 0.3f) },
            };
            ...
            _unitStyle = new GUIStyle(EditorStyles.miniLabel)
            {
                normal = { textColor = new Color(0.45f, 0.80f, 1.0f) },
            };
            ...
            {
                normal = { textColor = new Color(0.70f, 0.70f, 0.30f) },
            };
        }
        ...
        private void OnGUI()
        {
            ...
            if (_availableFmus.Count == 0)
                EditorGUILayout.HelpBox(
                    "No FMUs found. Click \"Refresh FMU List\" with the backend running.",
                    MessageType.Info
                );
            else
            {
                _selectedFmuIdx = EditorGUILayout.Popup(
                    "FMU",
                    _selectedFmuIdx,
                    _availableFmus.ToArray()
                );
                if (
                    GUILayout.Button(
                        "▶  Start Session for " + _availableFmus[_selectedFmuIdx],
                        GUILayout.Height(28)
                    )
                )
        ...
        private void DrawSession(FmuSession s, int idx)
        {
            ...
            {
                s.Collapsed = !EditorGUILayout.Foldout(
                    !s.Collapsed,
                    $"  {s.FmuName}  |  t = {s.SimTime:F3} s",
                    true
                );
            ...
            EditorGUILayout.LabelField(
                "Session ID",
                string.IsNullOrEmpty(s.SessionId) ? "—" : s.SessionId
            );
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(lbl, GUILayout.Height(24)))
                    s.AutoStep = !s.AutoStep;
            ...
            if (inputVars.Count > 0)
            {
                ...
                if (allBoolOn)
                    EditorGUILayout.HelpBox(
                        "⚠ All boolean inputs are active simultaneously — may cause conflicting states.",
                        MessageType.Warning
                    );
            ...
            bool anyVisible = s.PlotTimes.Keys.Any(k =>
                s.PlotTimes[k].Count >= 2 && (!s.SeriesVisible.ContainsKey(k) || s.SeriesVisible[k])
            );
            if (anyVisible)
            {
                GUILayout.Space(4);
                GUILayout.Label("Live Plot", _sectionStyle);
                Rect pr = GUILayoutUtility.GetRect(
                    GUIContent.none,
                    GUIStyle.none,
            ...
            switch (meta.Type)
            {
                ...
                default: // Real
                {
                    ...
                    if (!s.InputRanges.TryGetValue(name, out var range))
                    {
                        ...
                        {
                            float mag = Mathf.Max(1f, Mathf.Abs(startVal) * 2f);
                            lo = -mag;
                            hi = mag;
                        }
                    ...
                    float cur = s.InputValues.TryGetValue(name, out var rv)
                        ? Convert.ToSingle(rv)
                        : startVal;
                    DrawNumericRow(
                        name,
        ...
        )
        {
            using (new EditorGUILayout.VerticalScope("box"))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    GUILayout.FlexibleSpace();
                    range.UseSlider = EditorGUILayout.ToggleLeft(
                        "Slider",
                        range.UseSlider,
                        GUILayout.Width(56)
                    );
        ...
        {
            if (!s.SeriesVisible.ContainsKey(name))
                s.SeriesVisible[name] = true;
            if (!s.SeriesColorIdx.ContainsKey(name))
                s.SeriesColorIdx[name] = s.SeriesColorIdx.Count;

            Rect total = GUILayoutUtility.GetRect(
                48,
                16,
                GUILayout.Width(48),
                GUILayout.Height(16)
        ...
        private void DrawPlot(Rect r, FmuSession s)
        {
            ...
            {
                if (s.SeriesVisible.TryGetValue(name, out var vis) && !vis)
                    continue;
            ...
            if (vMin >= vMax || vMin == float.MaxValue)
            {
                vMin = -1;
                vMax = 1;
            }
            ...
            foreach (var name in s.PlotTimes.Keys)
            {
                ...
                if (visible && times.Count >= 2)
                {
                    ...
                    for (int i = 1; i < times.Count; i++)
                    {
                        ...
                        float y0 =
                            r.y + (1f - Mathf.InverseLerp(vMin, vMax, values[i - 1])) * r.height;
                ...
                string label =
                    vmeta != null && !string.IsNullOrEmpty(vmeta.Unit)
                        ? $"{name} [{vmeta.Unit}]"
                        : name;
        ...
        private async Task FetchVariablesAsync(FmuSession s)
        {
            ...
            foreach (var kv in s.Variables)
            {
                ...
                {
                    object def =
                        meta.Type == "Boolean" ? (object)false
                    ...
                    if (meta.Type == "Real")
                    {
                        ...
                        {
                            float mag = Mathf.Max(1f, Mathf.Abs(meta.Start ?? 0f) * 2f);
                            lo = -mag;
                            hi = mag;
                        }
        ...
        {
            if (s.Requesting || string.IsNullOrEmpty(s.SessionId))
                return;
            s.Requesting = true;
            ...
            if (!string.IsNullOrEmpty(json))
            {
                ...
                {
                    bool isInputSeries =
                        s.Variables.TryGetValue(name, out var vm)
                        && vm.Causality == "input"
                        && vm.Type == "Real";
                    ...
                    var tl = s.PlotTimes[name];
                    var vl = s.PlotValues[name];
                    if (tl.Count >= FmuSession.MaxSamples)
                    {
                        tl.RemoveAt(0);
                        vl.RemoveAt(0);
                    }
                ...
                foreach (var kv in s.Variables)
                {
                    if (kv.Value.Causality != "output" && kv.Value.Causality != "local")
                        continue;

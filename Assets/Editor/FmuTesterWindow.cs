...
namespace Speedma.Editor
{
    public class FmuTesterWindow : EditorWindow
    {
        ...
        private void EnsureStyles()
        {
            ...
            {
                normal = { textColor = new Color(1.0f, 0.85f, 0.1f) },
        ...
        private void DrawSession(FmuSession s, int idx)
        {
            ...
            if (inputVars.Count > 0)
            {
                ...
                foreach (var kv in inputVars)
                {
                    ...
                    switch (meta.Type)
                    {
                        ...
                        {
                            bool cur =
                                s.InputValues.TryGetValue(name, out var v) && Convert.ToBoolean(v);
                        ...
                        case "Integer":
                        {
                            int cur = s.InputValues.TryGetValue(name, out var v)
                                ? Convert.ToInt32(v)
                                : 0;
                        ...
                        default:
                        {
                            float cur = s.InputValues.TryGetValue(name, out var v)
                                ? Convert.ToSingle(v)
                                : 0f;
            ...
            if (s.OutputValues.Count > 0)
            {
                ...
                foreach (var kv in s.OutputValues)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        if (isReal)
                        {
                            // Área clicável maior: toggle + cor
                            ...
                            // Cor ao lado (verticalmente centrada)
                            int ci = s.OutputColorIdx.TryGetValue(name, out var c) ? c : 0;
                            Color col = PlotColors[ci % PlotColors.Length];
                            Rect colorRect = new Rect(
                                totalRect.x + 24,
                                totalRect.y + (totalRect.height - 12f) * 0.5f,
                                12,
                                12
                            );
                            EditorGUI.DrawRect(
                                colorRect,
                                visible ? col : new Color(col.r, col.g, col.b, 0.25f)
                            );

                            // Render do toggle (sem label)
                            bool next = EditorGUI.Toggle(toggleRect, visible);
                            if (next != visible)
                                s.OutputVisible[name] = next;

                            // Também aceita clique em qualquer ponto da área total (melhora usabilidade)
                            var e = Event.current;
                            if (
                            ...
                            {
                                bool toggled = !s.OutputVisible[name];
                                s.OutputVisible[name] = toggled;
                                e.Use();
                            }
            ...
            bool anyVisible = s.PlotTimes.Keys.Any(k =>
                s.PlotTimes[k].Count >= 2 && (!s.OutputVisible.ContainsKey(k) || s.OutputVisible[k])
            );
        ...
        private void DrawPlot(Rect r, FmuSession s)
        {
            ...
            {
                if (s.OutputVisible.TryGetValue(name, out var vis) && !vis)
                    continue;
        ...
        private async Task FetchVariablesAsync(FmuSession s)
        {
            ...
            {
                if (kv.Value.Causality != "input")
                    continue;
                s.InputValues.TryAdd(
                    kv.Key,
        ...
        private async Task StepAsync(FmuSession s)
        {
            ...
            if (!string.IsNullOrEmpty(json))
            {
                ...
                foreach (var name in s.PlotTimes.Keys)
                {
                    ...
                    if (tList.Count >= FmuSession.MaxSamples)
                    {
                        tList.RemoveAt(0);
                        vList.RemoveAt(0);
                    }
